...
一个可运行的 React/Vite 黑客松原型：系统根据一对匹配用户的公开聊天与当前场景允许的安全上下文生成双人破冰游戏。除了资料猜谜、转盘和极限二选一，AI 游戏工坊还会按 Prompt 编写完整 HTML/CSS/JavaScript，让双方直接玩投篮攻防、实时对抗、合作冒险或策略小游戏。
...
9. “资料猜谜局”为每个被猜的 TA 分别生成 3 组生活场景候选，双方各选三个标签，服务端固定拼成一句口语化印象，保密交接后共同揭晓，不生成分数或关系结论；
...
19. AI 未配置、超时、鉴权失败、限流或输出异常时自动回退同类型的安全本地小游戏代码，现场仍可真实操作。
...
25. `prompt-arcade` 支持“写 Prompt → AI 生成完整代码 → 沙箱试玩 → 用这个精确版本发邀请”，邀请绑定 artifact 指纹与一次性预览令牌；
26. 生成页面运行在无同源权限、无网络能力的 iframe 沙箱中；父页面只接受 PairPlay v1 白名单消息，不 `eval`、不把代码注入主页面；
27. 正式联机的角色、物理、倒计时、操作顺序、比分与胜负全部由服务端保存和推进；生成代码只负责画面、动画和采集允许的操作。
